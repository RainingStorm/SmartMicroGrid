...
class MicroGrid(gym.Env):
	...
	def step(self, action):
        ...
        # 奖励计算
		self.reward = (rg + rdg1 + rdg2 + rreg1 + rreg2) * 0.001#  + 0.583
		# print(rg)
	...
	def reset(self):
		...
		# 外部数据加载
		ra_list = [1, 5, 7, 14]
		ra = random.choice(ra_list)
		# ra = random.randint(1, 28)
		ra = 1
		load_path = './data/Load/load_'+str(ra)+'.csv'
		solar_path = './data/Solar/solar_' + str(ra) + '.csv'
		wind_path = './data/Wind/wind_' + str(ra) + '.csv'
		lmp_path = './data/LMP/lmp_' + str(ra) + '.csv'
		self.load = np.loadtxt(load_path, dtype=np.float64, delimiter=',', unpack=True) / 1000
		# print(len(self.load))
		self.solar = np.loadtxt(solar_path, dtype=np.float64, delimiter=',', unpack=True) / 1000
		self.wind = np.loadtxt(wind_path, dtype=np.float64, delimiter=',', unpack=True) / 1000
		self.lmp = np.loadtxt(lmp_path, dtype=np.float64, delimiter=',', unpack=True) / 10
		...
		# 加入随机噪声
		for i in range(24):
			self.P_net[i] += random.uniform(-10, 10)
			self.LMP[i] += random.uniform(-1, 1)
		'''
