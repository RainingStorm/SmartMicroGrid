...
def train(cfg, env, agent, eval_env):
    ...
    for i_ep in range(cfg.train_eps):
        ...
        if ra == 5:
            rewards_1.append(ep_reward)
            if ma_rewards_1:
                ma_rewards_1.append(0.9 * ma_rewards_1[-1] + 0.1 * ep_reward)
            else:
                ma_rewards_1.append(ep_reward)
    ...
    return rewards_1, rewards, ma_rewards, ma_rewards_1
...
if __name__ == "__main__":
    ...
    rewards_1, rewards, ma_rewards, ma_rewards_1 = train(cfg, env, agent, eval_env)

    # save results
    np.savetxt('./reward/rewards1.csv', rewards_1, fmt='%.2f', delimiter=',')
    np.savetxt('./reward/rewards.csv', rewards, fmt='%.2f', delimiter=',')
    np.savetxt('./reward/ma_rewards.csv', ma_rewards, fmt='%.2f', delimiter=',')
    np.savetxt('./reward/ma_rewards_1.csv', ma_rewards_1, fmt='%.2f', delimiter=',')
    ...
    plt.plot(rewards_1, label="$Train-rewards$")
    plt.plot(ma_rewards_1, label="$ma-rewards$")
    plt.legend()
    plt.show()
