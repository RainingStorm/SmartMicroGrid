...
class TDQN:
    def __init__(self, state_dim, actionD_dim, actionC_dim, max_action, min_action, cfg, env):
        ...
        self.memory = ReplayBuffer(cfg.memory_capacity, cfg.priority_alpha, cfg.priority_beta_start, cfg.priority_beta_frames)
    ...
    def evaluate(self, state):
        with torch.no_grad():
            ...
            q1, q2 = self.critic(state, c_a)
            # q_values = torch.min(q1, q2)
            q_values = (q1 + q2) / 2  # 在确定策略的环节，离散动作选择考虑Q值加权
    ...
    def select_discrete_action(self, state, continuous_action):
        ...
        if random.random() > self.epsilon(self.frame_idx):
            with torch.no_grad():
                ...
                # q_values = torch.min(q1, q2)
                q_values = (q1 + q2) / 2  # 在训练环节，离散动作选择考虑Q值加权
                discrete_action = q_values.max(1)[1].item()
        else:
    ...
    def update(self):
        ...
        state, discrete_action, continuous_action, reward, next_state, done = zip(*samples)
        ...
        diff1 = current_Q1 - expected_q_values.detach()
        diff2 = current_Q2 - expected_q_values.detach()
        diff = (diff1 + diff2) / 2
        self.memory.update_priorities(indices, diff.squeeze(1).detach().abs().cpu().numpy().tolist())
        loss = (diff1.pow(2) + diff2.pow(2)) * weights.to(self.device)
